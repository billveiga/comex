...
public class RelatorioSintetico {
    ...
    CategoriasProcessadas categoriasProcessadas = new CategoriasProcessadas();
    	...
    	public RelatorioSintetico(List<Pedido> listaPedidos) {
            super();
            if (listaPedidos == null || listaPedidos.isEmpty()) throw new IllegalArgumentException("A lista de pedidos não pode ser vazia!");

            this.totalDePedidosRealizados = listaPedidos.size();
            Function<Pedido, BigDecimal> mapaPedidos = pedido -> pedido.getValorTotal();
           
            this.totalProdutosVendidos = listaPedidos
            		.stream()
            ...
            this.montanteDeVendas = listaPedidos
            		.stream()
            		.map(mapaPedidos) 
    	...
    	public Pedido getPedidoMaisCaro(List<Pedido> listaPedidos) {
            return listaPedidos.stream().max(Comparator.comparing(Pedido::getValorTotal)).orElseThrow(() -> new IllegalStateException("A lista de pedidos não deveria estar vazia."));
        }
	...
	public Pedido getPedidoMaisBarato(List<Pedido> listaPedidos) {
        return listaPedidos.stream().min(Comparator.comparing(Pedido::getValorTotal)).orElseThrow(() -> new IllegalStateException("A lista de pedidos não deveria estar vazia."));
    }

	public BigDecimal getMontanteVendas(List<Pedido> listaPedidos) {
        Function<Pedido, BigDecimal> mapaPedidos = pedido -> pedido.getValorTotal();
        return montanteDeVendas = listaPedidos
                .stream()
                .map(mapaPedidos)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
	}
	...
	public Object getTotalProdutosVendidos(List<Pedido> listaPedidos) {
        return this.totalProdutosVendidos = listaPedidos
                .stream()
                .mapToInt(pedido -> pedido.getQuantidade())
                .sum();
    }
